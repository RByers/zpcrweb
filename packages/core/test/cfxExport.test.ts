 ...
 * (`threshold.md` §A.3). Cy5 has no well with a Cq, so its threshold is unrecoverable and it is
 * not exercised here.
 */
...
describe("CFX's own exported results", () => {
  ...
  it("reports a Cq for a noise well that touches the threshold once, and none for a well that never crosses", () => {
    ...
    // — the reference has no such gate, in either direction (`threshold.md` §A.4).
