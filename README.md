...
There is also limited support for `.pltd` and `.pcrd` files as produced by CFX software, but they are [encrypted](docs/zipcrypto.md) and this repo does not include the key (DMCA compliance). However you can easily exact the key from a copy a CFX Master using an AI agent like Claude code.
