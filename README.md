...
| [`melt.md`](./docs/melt.md) | Melt curves — the temperature ramp run after amplification, and the melting temperature (Tm) that identifies what was amplified. Not a file format doc. §1 states why the answer is a peak of the derivative rather than anything on the curve itself; §2 is the detection rule — **a melt is a plate-read step whose reads sweep temperature**, read from the reads alone, needing no `.prcl`, no `MELT` directive and no password; §3 the temperature axis, §4 the Savitzky–Golay smoothing and −dF/dT, §5 the peak and the one gate that keeps an empty well from being handed a confident Tm, §6 why a file that ships its own derivative (Biomeme) still produces one shared answer rather than a file-versus-computed toggle, §7 what is deliberately not done. Implemented by `packages/core/src/melt.ts` (detection) and `packages/core/src/meltAnalysis.ts` (entry point `computeMeltAnalysis()`). **Appendix A is the measurement** — 0.01 °C against 29.88 °C separating the two steps of one run, a 0.120 °C Tm spread across 27 replicates, and agreement with the Biomeme device's own peak to 0.21 °C; Appendix B records the signal-to-noise gate that looked obvious and does not work. |
