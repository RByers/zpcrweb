...
| [`prcl.md`](./prcl.md) | The `.prcl` thermal-cycling protocol files — lid/volume settings plus the ordered step list (hold, gradient, melt, goto, plate read), in the same encrypted-ZIP container as `.pltd`/`.pcrd`. The same `protocol2` XML document `.pcrd` embeds. **Documented but not yet implemented** — there is no `prcl.ts`. |
...
validated against the committed samples in `samples/` — though `prcl.md` documents a format that
is not implemented yet, and one field (the `PLATEREAD` operand) remains uninterpreted.
