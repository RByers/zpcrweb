...
| [`usb.md`](./usb.md) | The CFX96/C1000 instrument's own USB control protocol — not a file format, and not yet implemented anywhere in this repo. Documents what would be needed for a future WebUSB client: enumeration, the 5-byte application-layer frame, the ASCII command channel, and the file-upload/download mechanism a run is actually loaded and read back through. Reverse-engineered from USB captures, decoded with `tools/usbpcap_decode.py`. |
