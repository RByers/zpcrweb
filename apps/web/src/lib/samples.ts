 ...
 * the repo's `samples/` directory, so a file added to (or removed from) that directory shows up
 * in (or leaves) the app with no source change anywhere — on the next build, or immediately in a
 * running dev server, which watches the directory and reloads.
